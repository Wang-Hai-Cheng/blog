...
import static com.blog.model.constant.NoteBlogV4.Init.INIT_NOT;
import static com.blog.model.constant.NoteBlogV4.Init.INIT_STATUS;

/**
 * created by wang-hai-cheng on 2018/8/1 at 20:25
 *
...
public class MenuListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private void setUpMenuSystem(Object[][] folderMenus) {
        ...
        for (int i = 0; i < folderMenus.length; i++) {
            ...
            NBSysMenu parentMenu = fixMenu(parent[1].toString(), parent[0].toString(), i + 1, rootId, roleId, NBSysMenu.MenuType.PARENT);
            ...
            for (int j = 0; j < leafMenus.length; j++) {
                ...
                NBSysMenu lm = fixMenu(leafMenu[2], leafMenu[1], j, genMenuId, roleId, NBSysMenu.MenuType.LEAF);
    ...
    private void saveTopMenu(String icon, String name, String url, long rootId, long roleId) {
        NBSysMenu menu = fixMenu(icon, name, 0, rootId, roleId, NBSysMenu.MenuType.PARENT);
