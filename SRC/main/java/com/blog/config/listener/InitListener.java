...
import com.blog.dao.repository.*;
import com.blog.model.entity.permission.NBSysMenu;
import com.blog.model.entity.permission.NBSysResource;
import com.blog.model.entity.permission.NBSysRole;
import com.blog.model.entity.permission.NBSysRoleResource;
...
import static com.blog.model.constant.NoteBlogV4.Init.INIT_STATUS;
import static com.blog.model.constant.NoteBlogV4.Init.*;
import static com.blog.model.constant.NoteBlogV4.Param.*;

/**
 * spring boot 容器启动完成之后
 * 创建完表之后，插入一些初始值
 * created by wang-hai-cheng on 2018/7/15 at 17:14
 *
...
public class InitListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent applicationReadyEvent) {
        log.info("「个人博客」App 正在准备，请稍后...");
        long roleCnt = roleRepository.count();
        //没有初始化角色信息
        if (roleCnt == 0) {
            log.info("「个人博客」App 正在初始化权限系统，请稍后...");
            setUpAuthority(false);
            log.info("「个人博客」App 权限系统初始化完毕...");
        } else {
        ...
        if (panelCnt != NBPanel.PanelDom.values().length) {
            log.info("「个人博客」App 正在初始化首页右侧面板设置，请稍后...");
            panelRepository.deleteAll();
            setUpPanel();
            log.info("「个人博客」App 首页右侧面板初始化完毕...");
        }
        ...
        if (nbParam == null || StringUtils.isEmpty(nbParam.getValue()) || paramRepository.count() == 1) {
            ...
            setUpAppInitialText();
            log.info("「个人博客」App 参数初始化完毕！");
        } else {
            log.info("「个人博客」App 已经完成初始化，略过初始化步骤。");
        }
    }
    ...
    private void setUpAppInitialText() {
        String[][] words = new String[][]{
                ...
                {PROJECT_TOP_NOTICE, INIT_PROJECT_WORD, "项目置顶公告", "10"},
